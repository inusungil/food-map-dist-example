...
let url = "https://www.inuinu.shop";    //리버스프록시 이후에 변경 
...
btnSignOut.addEventListener("click", signOut);      // 로그아웃 버튼 이벤트 연결
...
async function setHeader(jwt) {
  ...
  const nickname = jwtReturn.data.result.nickname;    //파싱
  ...
  divUnsigned.classList.add("hidden");     // 태그에 클래스 추가
  divSigned.classList.remove("hidden");    // 태그에 클래스 삭제
